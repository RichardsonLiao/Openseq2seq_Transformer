...
class FullyConnectedTimeDecoder(Decoder):
  ...
  def _decode(self, input_dict):
          ...
          the following fields::
            input_dict = {
              'encoder_output': {
                ...
                "src_lengths": tensor with shape [batch_size]
              }
            }
    ...
    print('input_dict')
    print(input_dict)
    inputs = input_dict['decoder_output']['outputs']
    #inputs = input_dict['encoder_output']['outputs']
    regularizer = self.params.get('regularizer', None)

    batch_size, _, n_head = inputs.get_shape().as_list()
    ...
    if 'logits_to_outputs_func' in self.params:
      ...
      return {
          'outputs': outputs,
          'logits': logits,
          'src_length': input_dict['encoder_output']['src_lengths'],
      }
...
class FullyConnectedSCDecoder(Decoder):
  ...
  def _decode(self, input_dict):
    ...
    """
    inputs = input_dict['encoder_output']['outputs']
    lengths = input_dict['encoder_output']['src_lengths']
    regularizer = self.params.get('regularizer', None)
